...
Pushing to `main` syncs to the live storefront. Your branch is `claude/website-production-audit-yv6dkg` — develop and push there, not to `main`.
...
1. `git fetch origin main && git log origin/main --oneline -5` — check for `shopify[bot]` commits newer than your branch point. If any exist, rebase onto `origin/main` first so you are not editing stale content.
...
5. **F3** — in `templates/product.json`, keep the heading `SHIPPING & RETURNS` and replace its body with exactly:
   ...
   `<p>Standard shipping is $8, free on orders over $70. Express is $15. Orders ship from Ohio, and we get them out the door as quickly as we can after roasting. International rates are calculated at checkout.</p>`

   Note what this does **not** say: it makes no returns promise. See Stop Conditions.
...
10. Commit and push to `claude/website-production-audit-yv6dkg`.

## Stop Conditions

Stop, set Status to `Blocked`, record it in the Execution Report, and ask the user if any of these occur:

- **The returns policy is undefined.** Step 5 deliberately omits a returns promise because none exists in this project — Shop Policies has no Refund policy, and the benchmark's "30-Day Money Back Guarantee" was never adopted as ours. If you find yourself wanting to write a returns window, a guarantee, or a satisfaction promise: stop. That is a business commitment, not copy.
- **Shipping numbers don't match.** Step 5 states $8 standard / free over $70 / $15 express, read from the store's delivery profile on 2026-07-27. If you check and the live rates differ, stop rather than publishing a number that contradicts checkout.
...
- [ ] `git diff origin/main --stat` shows exactly two files changed
