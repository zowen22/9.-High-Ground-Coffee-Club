...
| P0-5 | **Owner's street address and phone are published** in the live privacy policy (`1150 Overlook Dr, Alliance, Ohio 44601`, `13306142860`), auto-filled from the shop's billing address, against the owner's stated preference. Owner asked for removal 2026-07-27; **no API path exists** — the Admin GraphQL `Mutation` type has no store-details/address/phone mutation (enumerated and confirmed). Admin UI only. Likely a replace-with-PO-box rather than a delete, since Shopify requires a business address. | Rendered policy URL + `Mutation` type enumeration, 2026-07-27 | @user (Settings → Store details) |
...
|---------|--------|----------|---------------|
| HO-01 — Purge Vessel demo copy | P1-1, P1-2, P1-3, P2-8 | High | **Done** (2026-07-27) |
